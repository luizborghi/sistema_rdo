...
$where_fiscal = "";
if ($_SESSION['usuario_tipo'] == 'fiscal') {
    $where_fiscal = " WHERE fiscal_id = {$_SESSION['usuario_id']}";
}

$stats = [
    'total' => $db->query("SELECT COUNT(*) FROM obras" . $where_fiscal)->fetchColumn(),
    'em_andamento' => $db->query("SELECT COUNT(*) FROM obras WHERE status = 'em_andamento'" . 
        ($_SESSION['usuario_tipo'] == 'fiscal' ? " AND fiscal_id = {$_SESSION['usuario_id']}" : ""))->fetchColumn(),
    'concluidas' => $db->query("SELECT COUNT(*) FROM obras WHERE status = 'concluida'" . 
        ($_SESSION['usuario_tipo'] == 'fiscal' ? " AND fiscal_id = {$_SESSION['usuario_id']}" : ""))->fetchColumn(),
    'pausadas' => $db->query("SELECT COUNT(*) FROM obras WHERE status = 'pausada'" . 
        ($_SESSION['usuario_tipo'] == 'fiscal' ? " AND fiscal_id = {$_SESSION['usuario_id']}" : ""))->fetchColumn()
];
?>
<!DOCTYPE html>
    ...
    <div class="container-fluid mt-4">
        ...
        <?php if ($_SESSION['usuario_tipo'] == 'fiscal'): ?>
            <div class="alert alert-info">
                ...
                <strong>Modo Fiscal:</strong> Você está visualizando apenas as obras onde você é o fiscal responsável.
            </div>
        <?php endif; ?>

        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2><i class="bi bi-building"></i> <?= $_SESSION['usuario_tipo'] == 'fiscal' ? 'Minhas Obras' : 'Gerenciar Obras' ?></h2>
